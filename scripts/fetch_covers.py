...
def get_wiki_image(title):
    ...
    try:
        with urllib.request.urlopen(request) as response:
            ...
            match = re.search(
                r'<table class="[^"]*infobox[^"]*".*?<img.*?src="(//upload\.wikimedia\.org/wikipedia/en/.*?)"',
                html,
                re.IGNORECASE | re.DOTALL,
            )
            if match:
                return 'https:' + match.group(1)
    except Exception as exc:  # pragma: no cover - network-sensitive path
        print(title, 'Error:', exc)
    return None


def convert(img, out_path, var_name):
    img = img.convert('RGB').resize((100, 100), Image.Resampling.LANCZOS)
...
for name, wiki_title in games.items():
    ...
    if img_url:
        ...
        try:
            ...
            print(f'Error downloading image for {name}: {exc}')
