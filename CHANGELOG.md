...
- **SD Card Catalog Scaling (2,048 ROMs in PSRAM)**: Scaled firmware ROM index from a hardcoded 100 entries to 2,048 entries allocated dynamically in Octal PSRAM (`MALLOC_CAP_SPIRAM`), consuming 0 bytes of internal SRAM. Added +/-10 rapid page-jump navigation (UP/DOWN buttons) to game selection menu in `BmoGameboy.ino`.
- **Automated Full-Catalogue Installer**: Created [`scripts/auto_install_romsets.py`](file:///e:/BMO%20Gameboy/scripts/auto_install_romsets.py) to automatically download, extract, sanitize filenames (< 60 chars), and stage complete 1G1R game sets for Game Boy, Game Boy Color, NES, and Doom into `games/` (and updated `.gitignore`).
